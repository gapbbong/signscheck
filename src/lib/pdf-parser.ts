...
export function extractNamesFromStructuredData(items: PDFTextItem[]): string[] {
    ...
    const stopWords = new Set([
        ...
        "디지털", "선도학", "동의서", "취지와", "운용내", "이해하", "교원으", "자발적", "참여할",
        "동의합", "사업운", "필요한", "범위내", "소속부", "학번학", "연락처", "기본인", "사항사",
        "수업연", "활동내", "결과물", "정보가", "관련법", "교육청", "개인정", "처리지", "따라수", "집이용",
        "본인은", "해당사", "학기말", "방학중", "학기중", "교육부", "교육청", "학교명"
    ]);

    return Array.from(potentialNames).filter(name => {
        if (stopWords.has(name)) return false;
        if (name.length < 2 || name.length > 4) return false;
        // Check if name contains any stopword as a substring
        for (const stop of stopWords) {
            if (name.includes(stop) || stop.includes(name)) return false;
        }
